...
open class FilterVC: UIViewController {

    public var delegate: FilterVCDelegate?
    ...
    private func configureLayout() {
        ...
        collectionView.register(UINib(nibName: "FilterCollectionViewCell", bundle: Bundle(for: self.classForCoder)), forCellWithReuseIdentifier: "FilterCollectionViewCell")
        ...
        let doneTop = saveButton.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor)
        doneTop.constant = 8
        doneTop.isActive = true
        let doneTrailing = saveButton.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor)
        doneTrailing.constant = -20
        doneTrailing.isActive = true
        saveButton.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(doneButtontapped)))
    ...
    private func applyFilterImageView() {
        ...
        DispatchQueue.global().async { [weak self] in
            let filterd = FilterService.shared.applyFilter(with: self?.image ?? UIImage(), filter: filter)
...
extension FilterVC: UICollectionViewDataSource, UICollectionViewDelegate
{
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        DispatchQueue.global().async { [weak self, weak cell] in
            ...
            filteredImage = FilterService.shared.applyFilter(with: filteredImage!, filter: filter)
            DispatchQueue.main.async {
